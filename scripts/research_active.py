...
VARIANTS = ("stage1", "stage12", "stage123", "stage1234")
CUMULATIVE_WEIGHTS = {
    "stage1": ("0.40", "0.40", "0.40", "0.40"),
    "stage12": ("0.40", "0.70", "0.70", "0.70"),
    "stage123": ("0.40", "0.70", "0.90", "0.90"),
    "stage1234": ("0.40", "0.70", "0.90", "1.00"),
}
...
def load_research_config(variant: str):
    ...
    cumulative = tuple(Decimal(value) for value in CUMULATIVE_WEIGHTS[variant])
    return replace(
        config,
        ...
        position=replace(config.position, cumulative_weights=cumulative),
    )


def stage_counts(booster_results) -> dict[str, int]:
    counts = {"stage1": 0, "stage2": 0, "stage3": 0, "stage4": 0}
    for result in booster_results.values():
        for cycle in result.closed_cycles:
            entry_count = int(cycle.get("entry_count", 0))
            for stage in range(1, min(entry_count, 4) + 1):
...
def run(variant: str, output: str) -> None:
    ...
    payload["research"] = {
        "variant": variant,
        "cumulative_weights": [float(value) for value in config.position.cumulative_weights],
        "closed_cycle_stage_counts": stage_counts(booster_results),
    }
...
def summarize(path: str) -> None:
    ...
    print(f"- Cumulative weights: {research['cumulative_weights']}")
    ...
    counts = research["closed_cycle_stage_counts"]
    print(
        "- Closed-cycle stage reaches: "
        f"S1={counts['stage1']}, S2={counts['stage2']}, "
        f"S3={counts['stage3']}, S4={counts['stage4']}"
    )
    print("| 반기 | 수익률 |")
    print("|---|---:|")
