...
이 디렉터리는 JDSS V3.1 TWIN-H40-S3의 개발·운영 문서와 과거 연구 기록을 함께 보관한다. 다른 환경의 Codex, ChatGPT, Antigravity는 아래 순서로 읽어야 한다.
...
| [`JDSS_FINAL_SPEC.md`](JDSS_FINAL_SPEC.md) | JDSS-3.1.0-TWIN-H40-S3 전략·주문·자금관리 계약 | 현재 전략의 최종 기준 |
...
| [`research/PR27_SIMPLE_STRATEGY_RESEARCH.md`](research/PR27_SIMPLE_STRATEGY_RESEARCH.md) | PR #27 후보 비교와 V3 초기 승격 근거 | 연구 이력 |
...
`Archive`, `Legacy`, `v1.x`, `v2.0`, `V3.0`으로 표시된 내용은 연구 재현과 의사결정 이력 보존용이다. 그 안의 과거 추세기간·부스터 상한·4차 추가매수·+6% TP2·20거래일 잔여청산 같은 값은 현재 전략에 적용하지 않는다. 현재 수치가 충돌하면 `strategy.yaml`과 `JDSS_FINAL_SPEC.md`를 우선하고, 둘이 서로 다르면 코드를 변경하기 전에 불일치로 보고한다.
