...
| `/backtest` 또는 `/bt` | 아래 제5절 형식 | 현재 production 포트폴리오 기준 백테스트 |
...
시작 전에는 관리 포지션·allocation 미체결 주문·SAFE_MODE 등 안전조건을 확인합니다. 조건이 맞으면 `1차 진입 시작 · 50%` 버튼이 표시됩니다. 다음 단계는 자동으로 열리지 않으며, 대기 거래일이 지난 뒤 `/onboarding`에서 운영자가 직접 엽니다.
