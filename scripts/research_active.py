...
VARIANTS = ("main_current", "evolved_no_rebuy", "evolved_rebuy")
...
def patch_tp1_ratio(ratio: Decimal) -> None:
    ...
    def adjusted(average_price, quantity, atr_pct, config, tick_size=Decimal("0.01")):
        ...
        tp1_qty = int((Decimal(quantity) * ratio).to_integral_value(rounding=ROUND_CEILING))
...
def load_research_config(variant: str):
    ...
    return replace(
        ...
        rebuy=replace(config.rebuy, enabled=(variant == "evolved_rebuy")),
    )
...
def run(variant: str, output: str) -> None:
    ...
    rebuy_fills = sum(
        1
        for symbol_result in booster_results.values()
        for trade in symbol_result.trades
        if trade.get("purpose") == "REBUY_CANDIDATE"
    )
    payload = result.to_dict()
    payload["research"] = {
        "variant": variant,
        "rebuy_enabled": bool(config.rebuy.enabled),
        "rebuy_fills": rebuy_fills,
    }
...
def summarize(path: str) -> None:
    ...
    print(f"- Rebuy Enabled: {research.get('rebuy_enabled', False)}")
    print(f"- Rebuy Fills: {research.get('rebuy_fills', 0)}")
    print("| 반기 | 수익률 |")
    print("|---|---:|")
