...
def annual_returns(results, years):
    ...
    for year in years:
        ...
        out[str(year)] = {
            "combined_pct": (
                sum(symbol_returns.values()) / len(symbol_returns)
                if symbol_returns
                else None
            ),
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "reports" / "v2_main_vs_final.json",
    )
    args = parser.parse_args()

    main_config = load_config(ROOT / "strategy.yaml")
    final_config = _with_stage1_guard(
        _with_entry_score(_with_tp(main_config, "0.06"), 55)
    )
    ...
    report = {
        ...
        "main": {
            ...
            "combined": combined_metrics(
                main_results, main_config.backtest.annualization_days
            ),
        ...
        "final": {
            "description": (
                "Entry55, TP4/6, SOXL stage1 sector guard, "
                "TP1+20d avg+2% remainder exit"
            ),
            "combined": combined_metrics(
                final_results, final_config.backtest.annualization_days
            ),
    ...
    for year in years:
        key = str(year)
        main_return = report["main"]["annual_returns"][key]["combined_pct"]
        final_return = report["final"]["annual_returns"][key]["combined_pct"]
        if main_return is None or final_return is None:
            continue
        lines.append(
            f"| {year} | {main_return:+.2f}% | {final_return:+.2f}% | "
            f"{final_return-main_return:+.2f}%p |"
        )
    lines += ["", "## Full-period metrics", ""]
    for name in ("main", "final"):
        metrics = report[name]["combined"]
        lines.append(
            f"- {name.upper()}: CAGR {metrics['cagr_pct']:+.2f}%, "
            f"MDD {metrics['mdd_pct']:.2f}%, "
            f"max hold {metrics['max_holding_days_worst_symbol_including_open']}d, "
            f"cycles {metrics['closed_cycles']}"
        )
