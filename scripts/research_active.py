...
VARIANTS = ("baseline_40", "strong_45", "strong_50")
...
def load_research_config(variant: str):
    ...
    if variant == "baseline_40":
        capital = Decimal("8000")
        max_weight = Decimal("0.40")
        ordinary_ratio = Decimal("1.0")
    elif variant == "strong_45":
        capital = Decimal("9000")
        max_weight = Decimal("0.45")
        ordinary_ratio = Decimal("0.8888889")
    else:
        capital = Decimal("10000")
        max_weight = Decimal("0.50")
        ordinary_ratio = Decimal("0.80")

    # score_to_exposure() uses score_92_100 for Score >= 92.
    # Keep ordinary signals at the same $8,000 cap (40% of portfolio),
    # and permit the larger cap only for the strongest score bucket.
    exposure = replace(
        config.exposure,
        score_82_87=ordinary_ratio,
        score_88_91=ordinary_ratio,
        score_92_100=Decimal("1.0"),
        allow_cap_increase=True,
        allow_cap_decrease_during_cycle=False,
    )
    return replace(
        config,
        global_=replace(config.global_, capital_per_symbol=capital),
        portfolio=replace(config.portfolio, booster_max_weight=max_weight),
        exposure=exposure,
    )
