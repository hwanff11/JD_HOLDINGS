...
def production_open_active(result, index: pd.DatetimeIndex) -> pd.Series:
    """Whether booster exposure exists after next-open BUYs, before intraday TP sells."""
...
def target_with_overlay(base, active_tqqq, active_soxl, overlay):
    ...
    active = [symbol for symbol, enabled in (("TQQQ", active_tqqq), ("SOXL", active_soxl)) if enabled]
...
def metrics(equity, exposures, trades):
    ...
    monthly = equity.groupby(equity.index.to_period("M")).last().pct_change(fill_method=None).dropna()
    return {
        ...
        "calmar": round((((final / CAPITAL) ** (1 / years) - 1) / abs(maximum_drawdown(equity))), 3),
...
def simulate(rule, frames, active, start, end, fee, slippage, benchmark=False):
    ...
    pending = base if benchmark else target_with_overlay(base, bool(act["TQQQ"].loc[prior_ts]), bool(act["SOXL"].loc[prior_ts]), rule.overlay)
    ...
    for timestamp in sessions:
        ...
        equity_values.append(cash + liquidation)
        exposures.append(liquidation / (cash + liquidation) if cash + liquidation > 0 else 0)
        if not benchmark:
            month = str(timestamp.to_period("M"))
            if month != last_month:
                base = base_target(frames["QQQ"].loc[timestamp], rule.strong, rule.bear)
                last_month = month
            pending = target_with_overlay(base, bool(act["TQQQ"].loc[timestamp]), bool(act["SOXL"].loc[timestamp]), rule.overlay)
...
def rolling_three_year(candidate, benchmark):
    ...
    for start_year in years:
        ...
        c = candidate[(candidate.index.year >= start_year) & (candidate.index.year <= end_year)]
        b = benchmark[(benchmark.index.year >= start_year) & (benchmark.index.year <= end_year)]
        ...
        rows.append({"period": f"{start_year}-{end_year}", "candidate_cagr_pct": round(c_cagr, 2), "qqq_cagr_pct": round(b_cagr, 2), "excess_pp": round(c_cagr - b_cagr, 2)})
    return rows
...
def bootstrap_monthly(candidate, benchmark, seed=311):
    ...
    return {
        "months": len(diff),
        "five_year_outperformance_probability_pct": round(float((arr > 0).mean()) * 100, 2),
...
def main() -> int:
    ...
    raw = {symbol: source.daily(symbol, warmup, end) for symbol in ("SPY", "QQQ", "TQQQ", "SOXL", "SOXX", "SMH")}
    ...
    booster = {
        ...
        "SOXL": engine.run("SOXL", raw["SOXL"], raw["SPY"], raw["QQQ"], start="2011-01-03", end=end, slippage=float(config.backtest.default_slippage), sector_data={"SOXX": raw["SOXX"], "SMH": raw["SMH"]}),
    }
    index = raw["QQQ"].index.intersection(raw["TQQQ"].index).intersection(raw["SOXL"].index)
    active = {symbol: production_open_active(booster[symbol], index) for symbol in ("TQQQ", "SOXL")}
    baseline_fee = float(config.global_.buy_fee)
    baseline_slip = float(config.backtest.default_slippage)
    benchmark_metrics, benchmark_equity = simulate(None, frames, active, "2011-01-03", end, baseline_fee, baseline_slip, benchmark=True)
    ...
    center_equity = {}
    for rule in centers:
        result, equity = simulate(rule, frames, active, "2011-01-03", end, baseline_fee, baseline_slip)
    ...
    for strong in (1.50, 1.625, 1.75, 1.875):
        for bear in (0.25, 0.50, 0.75):
            for overlay in (0.025, 0.05, 0.075, 0.10):
                ...
                result, _ = simulate(rule, frames, active, "2011-01-03", end, baseline_fee, baseline_slip)
                neighborhood.append({"rule": rule.__dict__, "metrics": result})

    stress = {}
    for rule in (centers[0], centers[1], centers[2], centers[3]):
        stress[rule.name] = {}
    ...
    rolling = {name: rolling_three_year(eq, benchmark_equity) for name, eq in center_equity.items()}
    bootstrap = {name: bootstrap_monthly(eq, benchmark_equity) for name, eq in center_equity.items()}
    annual_comparison = {}
    for name, result in center_results.items():
        annual_comparison[name] = {
            year: {
                ...
                "excess_pp": round(value - benchmark_metrics["annual_returns_pct"].get(year, value), 2),
            }
    ...
    for name, result in center_results.items():
        ...
        print(name, result, "rolling_wins", wins, "/", len(rolling[name]), "bootstrap", bootstrap[name])
    print("STRESS")
    for name, cases in stress.items():
        print(name, {label: (m["cagr_pct"], m["mdd_pct"], m["sharpe"]) for label, m in cases.items()})
    top = sorted(neighborhood, key=lambda row: (row["metrics"]["cagr_pct"], row["metrics"]["mdd_pct"]), reverse=True)[:12]
    print("NEIGHBORHOOD TOP")
    for row in top:
        print(row["rule"], row["metrics"]["cagr_pct"], row["metrics"]["mdd_pct"], row["metrics"]["sharpe"])
    return 0
