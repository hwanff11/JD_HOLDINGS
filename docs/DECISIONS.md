...
## D-008 — Telegram 운영 UI와 백테스트 기본값

- 인자 없는 `/backtest`의 기본값은 SOXL 최근 300거래일로 고정한다.
- 백테스트 매매 내역은 모든 매수 신호, 매수 미체결, 매수·추가매수·재매수,
  TP1·TP2 매도를 시간순으로 표시한다.
- Telegram 가독성을 위해 종목당 최근 15건만 보여주고 생략 건수를 표시한다.
- 계좌 조회는 미국주식만 표시하고 토스의 수수료 반영 평가손익을 사용한다.
