...
- 실주문은 `JDSS_TRADING_MODE=live`와 정확한 `JDSS_LIVE_CONFIRMATION`이 동시에 있어야 한다.
...
- 실거래 잠금, 2단계 승인, 멱등성, Reconciliation 또는 SAFE_MODE를 약화하는 변경은 별도 승인 없이는 허용하지 않는다.
...
- [ ] 재시작·부분체결·오류 시 SAFE_MODE 또는 안전한 실패가 가능한가
